...
const Entregar = () => {
  ...
  const inicialCargado =
    "Arrastra y Suelta el archivo, o bien da click para buscar el archivo";
  ...
  const tiempoRestante = (fecha) => {
    ...
    const strRestante = `${dias} ${
      dias > 1 || dias == 0 ? "Dias" : "Dia"
    }  ${horas} ${horas >= 1 ? "Horas" : "Hora"}  ${minutos} Minutos`;
    return strRestante;
  };
  ...
  useEffect(() => {
    const existTarea = async (e) => {
      const data = await privateAxios
        ...
        .then((response) => {
          console.log(response.data.rslt);
  ...
  const clickEntregar = async (e) => {
    ...
    var error = false;
    const tarea = await privateAxios
      .put("/api/v1/tareas/newentregable", {
        params: {
  ...
  const clickActualizar = async (e) => {
    ...
    var error = false;
    const tarea = await privateAxios
      .put("/api/v1/tareas/updateentregable", {
        params: {
          idclas: "6225659f6208d12d1cdce801",
          idest: "622541aef1bd3418216d8831",
        },
  ...
  const clickEliminar = async (e) => {
    e.preventDefault();
    const del = await privateAxios
      .delete("/api/v1/tareas/deleteentregable", {
        params: {
          idclas: "6225659f6208d12d1cdce801",
          idest: "622541aef1bd3418216d8831",
          numeroTarea: numTarea,
      ...
      <div className="container">
        <div className="container-body">
          <div className="descripcion-tarea">
            ...
            <p className="puntaje">{puntaje} Puntos</p>
            <br />
            <h3 className="subtitulo">Tiempo Restante</h3>
            <p className="tiempo-restante">{tiempo}</p>
            <br></br>
            <h3 className="subtitulo">Estado</h3>
            <p className={entregada ? "entregado" : "pendiente"}>
              {entregada ? "Entregado" : "Sin Entregar"}
            </p>
          </div>
